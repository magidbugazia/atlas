...
5. **Explicit file lists for agents.** When spawning agents, always include the complete list of files they need to process. Do not make agents discover files. Give them the list.
   - **Path anchoring.** When the orchestrator (you) and any spawned agent make tool calls, every KB path (`wiki/...`, `raw/...`, `.atlas/...`, `KB.md`, `INDEX.md`) must be anchored to the KB root resolved by Phase 0. Do not use bare relative paths — your CWD may be a subdirectory of the KB root, in which case bare paths resolve incorrectly. Substitute every `[KB root]` placeholder in agent prompts with the absolute path from Phase 0 before spawning, and prefix every path in your own tool calls with that absolute path. Each agent prompt also carries an inline `(PATH RESOLUTION: ...)` reminder right after its `KB root: [path]` header.
