...
2. **`KB.md` is renamed to `README.md` in the temp dir.** code-fluent is a directory-scanning skill — it builds a guide from whatever it finds in a folder. If handed just a lone report `.md`, it has no project context (no subject, no domain). code-fluent's Phase 1 derives project identity from files at the target root (it now checks `KB.md`'s H1 at top priority as well as `README.md`, so the rename is partly redundant but kept for robustness: README.md works on any code-fluent version). Atlas's `KB.md` already holds the subject, categories, and scope. Renaming `KB.md` → `README.md` in the temp dir lets code-fluent read atlas's metadata natively, with no modification to code-fluent. This is the integration trick; do not remove it without replacing it with something equivalent.
