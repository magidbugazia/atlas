...
2. **Dedup check:** Grep `raw/` recursively for the URL string in file frontmatter. If found, tell the user: "This URL was already ingested as `[existing file path]` on [date]. Update the existing copy or add a duplicate?" If they say update, overwrite the existing file (this is the single user-approved exception to SKILL.md's raw-append-only rule; the changed hash correctly triggers recompile of the source). If they say skip, stop. Only proceed with a new file if they explicitly want a duplicate.
...
7. If the extracted markdown contains image references (`![alt](url)`), attempt to download each image:
   ...
   - For each significant image: download it via Bash python3 (`urllib.request.urlretrieve`; use a `/tmp` script via the Write tool for batches). WebFetch cannot do this — it returns LLM-processed text, not binary bytes — and `curl`/`wget` are hook-blocked on this machine. Save to `raw/images/[article-slug]-[N].ext` where N is a counter and ext matches the original format.
